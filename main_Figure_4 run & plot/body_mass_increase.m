...
% 计算体型为 Mm (total mass; C mass; N mass) 的捕食者取食个体大小为m_prey的增长速度
m = Mm(1);delta_C = Mm(2)/Mm(1);delta_N = Mm(3)/Mm(1);

% predator 生物量维持
B_maint = v_m * m;    % 参数 B_m

% 从食物中获取的能量，剩余的能量
A = E_c * m_prey * e;
B_rema = A - f*B_maint; 
% 如果能量不够维持生物量维持，则消耗自身mass
if B_rema < 0
    pii = (f*B_maint - A) / E_c;
else
    pii = 0;
...
% H_C(H_C<1e-10)=0; H_N(H_N<1e-10)=0;

% 元素是否满足持生物量维持需求，不够的话，需要消耗已经储存的C, N
if H_C == 0
...
% 考虑在合成新生物量过程中的元素消耗 (储存多余的C元素)
H_C_1 = H_C;
H_N_1 = min(H_N, (delta_N_pie/delta_C_pie) * H_C*(delta_N)^h*GGE_C/(delta_C)^h);

% 考虑在合成新生物量过程中的元素消耗 (储存多余的N元素)
% H_C_1 = min(H_C, (delta_C_pie/delta_N_pie) * H_N/GGE_C);
% H_N_1 = H_N;
   
% 考虑元素限制增加了单位生物量合成的能量消耗
